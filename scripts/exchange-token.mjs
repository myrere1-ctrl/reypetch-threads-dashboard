...
const account = (process.env.ACCOUNT || 'iams').toUpperCase();
...
console.log('---');
console.log(`Set GitHub Secrets untuk akun ${account}:`);
console.log(`  ${account}_THREADS_ACCESS_TOKEN =`, long.access_token);
console.log(`  ${account}_THREADS_USER_ID =`, userId);
console.log('');
console.log(`Command untuk set langsung via gh CLI:`);
console.log(`  gh secret set ${account}_THREADS_ACCESS_TOKEN --body "${long.access_token}"`);
console.log(`  gh secret set ${account}_THREADS_USER_ID --body "${userId}"`);
